...
Generator.prototype.normalizeJSON = function(obj) {
  ...
  var qte = this.bbb.get("indent").size || 1;
  var indentChar = (this.bbb.get("indent").char === "space") ? " " : "\t";
  var indent = "";
  while (qte--) {
    indent += indentChar;
  }
...
Generator.prototype.normalizeJS = function(code) {
  return beautify.js_beautify(code.toString(), {
    "indent_size"               : this.bbb.get("indent").size,
    "indent_char"               : this.bbb.get("indent").char === "space" ? " " : "\t",
    "indent_with_tabs"          : this.bbb.get("indent").char === "space" ? false : true,
