...
Generator.prototype.module = function module() {
  var output = this.normalizeJS(this.src.read("module." + this.bbb.get("moduleStyle") + ".js"));
  this.write(path.join(this.bbb.get("paths").base, this.bbb.get("paths").modules, this.moduleName + ".js"), output);
};
...
Generator.prototype.moduleTest = function moduleTest() {
  ...
  var testFW = this.bbb.get("testFramework");

  if (!testFW || testFW === "none") return;

  // Create test file
  var testFolder = path.join(this.bbb.get("paths").base, this.bbb.get("paths").tests, testFW);
  ...
  var dest = path.join(this.bbb.get("paths").base, this.bbb.get("paths").tests, testFW,
